...
grep -E "$short_dark_patt" sorted_darks.lis > darks_1s.lis
grep -E "$long_dark_patt" sorted_darks.lis > darks_long.lis
...
# Generate BPM:
reduce @flats.lis @darks_1s.lis -r makeProcessedBPM
...
# Reduce science data:
reduce @sci.lis -p addDQ:user_bpm=${bpm}
