...
class Spect(PrimitivesBASE):
    ...
    def determineWavelengthSolution(self, adinputs=None, **params):
        ...
        for ad in adinputs:
            ...
            rms = m.rms_output
            log.stdinfo(m_final)
            log.stdinfo("Matched {} lines with rms = {:.3f} nm.".format(len(m.input_coords), rms))
...
def _find_peaks(data, widths, mask=None, variance=None, min_snr=1, min_frac=0.25,
    ...
    if rank_clip:
        ...
        while len(rank_order) > diff and (peak_snrs[rank_order[-1]] /
            ...
            mask[peak-maxwidth:peak+maxwidth+1] |= DQ.bad_pixel
...
def _set_model(model, order=None, initial=False, kdfit=False):
    ...
    if kdfit:
        ...
        c0_unc = 50*dw if initial else 20*dw
