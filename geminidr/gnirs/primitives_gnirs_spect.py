...
class GNIRSSpect(Spect, GNIRS):
    ...
    def _get_arc_linelist(self, waves=None, ad=None):
        ...
        lim0 = (waves[0] + (waves[0] - waves[-1]) * 1.02 * 1.05) * 10  # higher limit
        lim1 = (waves[-1] - (waves[0] - waves[-1]) * 1.02 * 1.05) * 10
        print(f"Lim0={lim0}, lim1={lim1}")

        return wavecal.LineList(filename).copy_bracketed(lim1, lim0)
