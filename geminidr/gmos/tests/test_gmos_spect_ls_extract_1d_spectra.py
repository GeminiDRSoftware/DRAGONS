...
test_datasets = [
    # Input Filename
    ("process_arcs/GMOS/N20180508S0021_skyCorrected.fits",
...
def ad(request, ad_factory, path_to_outputs):
    ...
    ad_out = p.extract1DSpectra([_ad], method="standard", width=None, grow=10)[0]
...
def preprocess_recipe(ad, path, center, arc):
    ...
    ad = _p.traceApertures([_ad], trace_order=2, nsum=20, step=10,
                           max_shift=0.09, max_missed=5)[0]
    ad = _p.skyCorrectFromSlit([ad], order=5, grow=0)[0]
