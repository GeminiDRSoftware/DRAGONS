...
@pytest.mark.gmosls
@pytest.mark.parametrize("ad,start", datasets_and_locations, indirect=True)
def test_add_illum_mask_position(ad, start):
    p = GMOSLongslit([ad])
    p.prepare()
    ...
    start_row = ad[0].mask[200:,100].argmax() + 200
    assert abs(start_row - start) <= 2
