...
def makeProcessedArc(p):
    ...
    p.tileArrays()
    p.stackArcs()
    p.extractSpectra(flat_correct=False)
    p.determineWavelengthSolution()
    p.captureWfitInstMon()
    p.storeProcessedArc()
    return
...
def checkArcWfit(p):
    """
    This recipe extracts some values from the .WFIT extension for instrument
    monitoring. The values extracted are the polynomial coefficients of the
    reference order polynomial fit. They are placed in the header to be picked
    up by the instrument monitoring system.

    This recipe needs to be run on a GHOST processed arc.
    :param p:
    :return:
    """
    p.captureWfitInstMon()
