...
def dist_circle(n, xcen=-99 ,ycen=-99):
    ...
    x_2 = (np.arange(nx) - xcen) ** 2     #X distances (squared)
    y_2 = (np.arange(ny) - ycen) ** 2     #Y distances (squared)  
    im = np.empty((nx, ny))      #Make uninitialized output array

    for i in xrange(ny): 
       im[i,:] = np.sqrt(x_2 + y_2[i])     #Euclidian distance
