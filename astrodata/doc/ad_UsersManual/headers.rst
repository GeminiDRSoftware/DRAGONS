...
.. note::
   Of course, if the Descriptors have not been configured
   for the data's AstroDataType or if the AstroDataType for the data is not defined, the
   Descriptors will not work.  In that case, it is nevertheless possible to access the
   header information directly.  This is also shown later in this chapter.
...
.. warning::
   ``all_descriptor_names()`` will be replaced by the ``descriptors`` properties in
   the new AstroData API.
...
coded.  That's a more advanced topic addressed elsewhere (:ref:`advanced`). 
...
to get the value of a specific header keyword::
  ...
  naxis2 = ad['SCI',1].get_key_value('NAXIS2')
...
uses that concept extensively.  See :ref:`structure` for information on the typical structure of AstroData
...
.. warning::
   The ``get_headers()`` method will be replaced with the ``headers`` property in the
   new AstroData API.
...
extension, use the ``set_key_values()``.  The difference is that one has to specify the
extension ID in the latter case, or already be working on an extension. ::
  ...
  # When looping through the extensions, there's no need to specify the extension 
  # number since there's only one.  The arguments are *keyword*, *value*, *comment*, 
  # with comment being optional.
  for extension in ad['SCI']:
...
.. note::
   refer to Emma's document for complete instructions
