...
#                                                                     gemutil.py
# ------------------------------------------------------------------------------
# $Id: gemutil.py 5081 2014-12-12 18:27:34Z kanderson $
# ------------------------------------------------------------------------------
__version__      = '$Revision: 5081 $'[11:-2]
__version_date__ = '$Date: 2014-12-12 08:27:34 -1000 (Fri, 12 Dec 2014) $'[7:-2]
# ------------------------------------------------------------------------------
import os
import sys
import time
...
This module contains the following utilities:
    ...
    gemdate(zone='UT', timestamp = None)
    stdDateString(date)
    ...
    rename_hdu(name=None, ver=None, hdu=None)
"""
...
def imageName(image, rawpath='', prefix='auto', observatory='gemini-north',
               suffix=None, verbose=True):
    """
    Convert an image number to a filename using UT date. If image is already 
    a string simply check image's existence and MEFness.
    ...
    @param observatory: name of observatory, can be either 'gemini-north' or 
                        'gemini-south'
    ...
    if iraf.gemisnumber.fl_istype:
        if verbose: 
            print "# IMAGENAME - Constructing image name based on today's UT date."
...
def rename_hdu(name=None, ver=None, hdu=None):
    ...
    if name is None and "EXTNAME" not in nheader:
    ...
    if nheader.get("TFIELDS"): 
        kafter = "TFIELDS"
    if name:
        nheader.set("extname", name, "added by AstroData", after=kafter)
    if ver:
        nheader.set("extver", ver, "added by AstroData", after="EXTNAME")
