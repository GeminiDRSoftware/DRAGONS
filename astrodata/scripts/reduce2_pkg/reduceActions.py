...
Action classes provided:
    ...
    CalibrationAction     - user calibration services (--usercal)
...
class CalibrationAction(Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        # handle pos args already in namespace
        if _extant_pos_args and _pos_args:
            _extant_pos_args.extend(_pos_args)
            setattr(namespace, 'files', _extant_pos_args)
        elif _pos_args and not _extant_pos_args:
            setattr(namespace, 'files', _pos_args)
