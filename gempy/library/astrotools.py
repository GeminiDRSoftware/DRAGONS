...
def parse_user_bins(bins, frame_size:int):
    ...
    for bin in re.split(",|;| ", bins.strip("[]()'")):
        ...
        for bin_limit in bin_limits:
            if bin_limit.isdigit():
                if int(bin_limit) <= frame_size:
                    int_bin_limits.append(int(bin_limit))
                else:
                    int_bin_limits.append(frame_size)
            else:
                raise TypeError("Bin ranges must be integer")
        int_bin_limits.sort()

        # trim off the bins that are outside the frame pixel range
        if int_bin_limits[0] == frame_size:
            break
        if int_bin_limits[1] == frame_size:
    ...
    for i,bin in enumerate(bin_list[:-1]):
        if (len(bin_list) - 1) > i:
            new_list = bin_list[i+1:]
            for n,b in enumerate(new_list):
                if len(new_list) > n:
                    if bin_list[i][1] >= new_list[n][0]:
                        merged_bin = (bin_list[i][0],max(bin_list[i][1],new_list[n][1]))
                        bin_list[i] = merged_bin
                        del bin_list[i+1]
    # a hack to make bins compatible with how regions are displayed
    # this can go if the requirement for the user is to introduce regions
    # with region right end excluded
    adjusted_bin_list = []
    for i,bin in enumerate(bin_list):
        if i == 0 and bin_list[0][0] == 0:
            adjusted_bin = bin
        else:
            adjusted_bin = (bin_list[i][0]-1,bin_list[i][1])
        adjusted_bin_list.append(adjusted_bin)
    print(adjusted_bin_list)
