...
# Acceptance Tests
#
# This module puts together all the Aceptance Tests for the Mosaic project and
# it can be run in conjunction with the explanations in the MS Word document:
# ATPMosaic-Oct4.docx
...
try:
    import pyds9                     # Should come from anaconda. Not in Ureka.
except ImportError:
    pass
...
def at1():
    """
    This test creates a list of 4 ndarrays of the same size as input to
    MosaicData. The test checks that the data is not corrupted during this
    process by calculating the median of each ndarray before and after
    MosaicData instantiation.

    """
    print('\n at1 REQUIREMENT.......')
    print('MosaicData instantiates an object from a list of equal size ndarrays.')
    ...
    # expected: [-500.0, 500.0, -500.0, 500.0]
...
def at2():
    ...
    geometry = {
        'transformation': {
            'shift': [(0., 0.), (43.60, -1.24), (0.02, 41.10),(43.42, 41.72)],
    ...
    print('\n at2 REQUIREMENT.......')
    print('MosaicGeometry instantiates an object given a Geometry Dictionary')
    geo = MosaicGeometry(geometry)

    # Verify attributes for GSAO geometry values. geo.info() forms a dictionary
    # with the object attributes.
    print(geo.info())
    return
...
def at3():
    ...
    """
    print('\n at3 REQUIREMENT.......')
    print ('When position information is not available, Mosaic shall '
           'tile horizontally the list of input ndarrays.')
    ...
    # Create a valid object from data list
    md = MosaicData(data_list)

    # Create a Mosaic object using this MosaicData object
    mo = Mosaic(md)

    # Build a mosaic by tiling the input ndarrays; no geometry information.
    mosaic_data = mo.mosaic_image_data()

    # If using the above input data list, the output should be (2048,2048).
    print(mosaic_data.shape)

    list_average=[np.average(data_list[k]) for k in range(4)]
    print('Input data average is:',np.average(list_average))
    print('Expected: 0.0')
    print('Output mosaic average:',np.average(mosaic_data))
    print('Expected: 0.0')
    return

def at4():
    """
    This test creates a list of 4 ndarrays where one of them as a different
    shape than the other 3. This is input to MosaicData which raises an
    exception.

    """
    print('\n at4 REQUIREMENT.......')
    print ('Raise exception when given a list of ndarrays of different shapes')

    # Make an ndarray of shape (2048,1024) of pixel values between 0, 1000.
    data = np.linspace(0., 1000., 1024*2048).reshape(2048, 1024)
    data_list = [data*(-1)**k for k in np.arange(4)]
...
def at6():
    ...
    """
    print('\n at6 REQUIREMENT.......')
    print('Create a mask ndarray with value 1 for no-data areas')
...
def at7():
    ...
    The test creates a MosaicGeometry object from just 2 input values a
    blocksize and mosaic_grid tuples. Then it creates a MosaicData object with
    4 data ndarrays and a dictionary names 'coords' with coordinates information
    about the input data.

    Success Criterion 1): create coords['amp_mosaic_coord'] such that
    we have 1 amplifier per block. 

    Success Criterion 2): create coords['amp_mosaic_coord'] such that
    we have 2 amplifiers per block/

    """
    print('\n at7 REQUIREMENT.......')
    print('Return an ndarray from a given block number')
    ...
    #
    # Success Criterion 1.
    # Set 'amp_mosaic_coord' for Success Criterion 1)
    coords = {
        'amp_mosaic_coord': [
    ...
    print('Median value of input block(1,0):',np.median(data_list[1]))
    print('Median value of output block:', np.median(block_data))

    # (height, width) in pixels. This should be (2048,1024)
    ...
    data = np.linspace(0.,1000.,512*2048).reshape(2048,512)
    data_list = [data*(-1)**k for k in np.arange(8)]
    amp_mosaic_coord = []
    amp_block_coord = []
    coords={}
    for k in range(8):
...
def at8():
    """
    Create a 2 ndarrays list and mark a strip of the 2nd ndarray with a 
    higher value. Set the Geometry dictionary with 'rotate' this ndarray
    by 5 degrees. Now we create the mosaic with default interpolation function
    and again with the 'spline' function of order 5. We plot a column from each
    mosaic.
...
def atd2():
    ...
    a given name. The test uses the MosaicAD method mosaic_image_data  to create
    a mosaic using the  extension name 'SCI'.

    """
    print('\n atd2 REQUIREMENT.......')
    print ('given extension name in the input AstroData object, the system'
            ' shall create a mosaic')

    gmos_file = '../data/gS20120420S0033.fits'
    ad = astrodata.open(gmos_file)
    mo = MosaicAD(ad, gemini_mosaic_function)
    # Now use the mosaic_image_data method to create the mosaic ndarray.
...
def atd3():
    ...
    """
    print('\n atd3 REQUIREMENT.......')
    print('AstroData object with associated binary table, the system '
    ...
    ## @TODO N.B. Unfamiliar with new pyds9 api. Calls below are suspect.
    #
    # Display the image
...
def atd5():
    """
    Verify that mosaicAD gives the correct WCS information for the mosaiced
    data.
    ...
    """
    print('\n atd5 REQUIREMENT.......')
    print('Update the header keywords CRPIX1, CRPIX2 in output mosaiced AD '
          'object to match the requested transformations')
    gmos_file='../data/gS20120420S0033.fits'
    gsaoi_file='../data/guS20120413S0048.fits'
    ad = astrodata.open(gmos_file)
...
def atd6():
    ...
    with the block parameter value (0,0), indicating to output the lower left
    block.
    ...
    """
    print('\n atd6 REQUIREMENT.......')
    print('Create a block from a given extension name')
    gmos_file='../data/gS20120420S0033.fits'
    gsaoi_file='../data/guS20120413S0048.fits'
