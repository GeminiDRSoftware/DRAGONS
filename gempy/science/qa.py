...
def measure_zp(adinput=None):
    ...
    The reference magnitudes (refmag) are straight from the reference catalog
    The measured magnitudes (mags) are straight from the object detection catalog

    We correct for astromepheric extinction at the point where we calculate the 
    zeropoint, ie we define:
    actual_mag = zeropoint + instrumental_mag + extinction_correction

    where in this case, actual_mag is the refmag, instrumenal_mag is the mag 
    from the objcat, and we use the nominal extinction value as we don't have a measured on
    at this point. ie  we're actually computing zeropoint as:
