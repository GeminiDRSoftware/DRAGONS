...
def show_primitives(_file, mode='sq', recipe='reduce'):
    """
    show_primitives takes in a file, the observing mode, and the data reduction
    recipe name, and will return the source code pertaining to that recipe, straight
    from the source file. This source codes shows all the primitives that are used
    in that recipe, helping the user distinguish what primitives are being used.

    Parameters
    ----------
    file - str
        file name for the fits file, useful for the astrodata.tags, which are then
        used to determine what directory to find the recipe in.
    mode - str
        either 'qa' or 'sq' for the two modes available. Anything else will return
        an error. mode is also needed to navigate to the proper directory
    recipe - str
        name of the recipe. Must be exactly the same as the function name/recipe.
        str + mode find the python file where the recipe exists in, the parameter
    ...
    # First make sure mode is a valid input, otherwise will look for file that doesn't exist
    ...
    # a list of current instruments needs to be found so show_primitives can parse through
    ...
    # TODO: this is hardcoded and does not consider new types of modules. If that needs to be added, it is possible.
    if "DARK" in ad.tags:
        tag_object = "DARK"
        module = 'recipes_DARK'
    ...
    absolute_dir = 'geminidr.' + instrument + '.recipes.' + mode.lower() + '.' + module
    ...
    try:
        ...
        raise ModuleNotFoundError("The expected location of the recipe does not exist. "
                                  ...
                                  "This may mean the object type in the file provided does not have"
                                  " a recipe with the given mode and recipename parameters"
    ...
    # Used show_recipes to give user all possible recipes if the recipe provided does not exist
    functions_list = [i[0] for i in inspect.getmembers(mod) if inspect.isfunction(i[1])]

    # If recipe not provided/'default' input used, check to see default recipe exists, otherwise error
    if recipe is 'default':
        if 'default' in functions_list:
            print("Recipe was not provided, but file contained default recipe. "
                  "Show_primitives will use the default recipe.")
            recipe = 'default'
        else:
            raise ValueError("Recipe was not provided, and the module that corresponded "
                             "to the file and mode provided did not specify a default recipe "
                             "to use. Please make sure a recipe is provided. \n \n"
    ...
    try:
        ...
        raise ImportError("\n The requested recipe was not found in the expected module, more "
                          "likely than not, this is because the recipe name given "
    ...
    print("Matched recipe: {}".format(absolute_dir + "::" + recipe_in_module.__name__))
